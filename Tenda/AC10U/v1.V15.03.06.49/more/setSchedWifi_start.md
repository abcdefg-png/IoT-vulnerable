...
The Tenda AC10U v1.0 Firmware V15.03.06.49 firmware has a stack overflow vulnerability `schedStartTime` and `schedEndTime` parameters from the `setSchedWifi` function. The `sched_start_time` variable receives the `schedStartTime` parameter from a POST request and is later assigned to the `wlan_switch` variable, which is fixed at 25 bytes. However, since the user can control the input of `schedStartTime`, the statement `strcpy(wlan_switch->begin_time, sched_start_time);` can cause a buffer overflow. The user-provided `schedStartTime` can exceed the capacity of the `wlan_switch` array, triggering this security vulnerability.
